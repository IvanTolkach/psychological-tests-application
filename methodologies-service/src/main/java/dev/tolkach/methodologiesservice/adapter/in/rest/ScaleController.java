...
import dev.tolkach.methodologiesservice.adapter.in.rest.endpoint.ScaleEndpoint;
...
public class ScaleController implements ScaleEndpoint {
    ...
    @Override
    public ResponseEntity<ScaleDto> getScaleById(UUID scaleId) {
        Scale scale = scaleUseCase.getScaleById(scaleId);
        ScaleDto responseDto = scaleDtoMapper.toDto(scale);
